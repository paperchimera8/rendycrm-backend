...
export function useSlotEditor(date: string) {
  return useQuery({
    queryKey: ["slot-editor", date],
    queryFn: () => getSlotEditor(date),
  });
}

export function useWeekSlots(date: string) {
  return useQuery({
    queryKey: ["week-slots", date],
    queryFn: () => getWeekSlots(date),
  });
}

export function useAvailableSlots(dateFrom: string, dateTo: string) {
  return useQuery({
    queryKey: ["available-slots", dateFrom, dateTo],
    queryFn: () => getAvailableSlots(dateFrom, dateTo),
  });
}
...
export function useOperatorBotSettings() {
  return useQuery({
    queryKey: ["operator-bot"],
    queryFn: getOperatorBotSettings,
  });
}
...
export function useCancelBookingMutation() {
  return useMutation({
    mutationFn: ({
      ...
      id: string;
      conversationId?: string;
    }) => cancelBooking(id, conversationId),
  });
}
...
export function useReorderDaySlotsMutation() {
  return useMutation({
    mutationFn: ({ slotDate, ids }: { slotDate: string; ids: string[] }) =>
      reorderDaySlots(slotDate, ids),
  });
}
...
export function useUpdateMasterProfileMutation() {
  const client = useQueryClient();
  return useMutation({
    mutationFn: ({ masterPhone }: { masterPhone: string }) =>
      updateMasterProfile(masterPhone),
