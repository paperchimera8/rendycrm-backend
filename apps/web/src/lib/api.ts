...
async function request<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  try {
    ...
    if (
      ...
      throw new Error(
        `Не удалось выполнить запрос ${path}. Проверьте backend и dev proxy.`,
      );
    }
    throw error;
  }
...
export async function getConversations() {
  const data = await request<{ items?: Conversation[] | null }>(
    "/conversations",
  );
...
export async function getSlotEditor(date: string) {
  const data = await request<Partial<SlotEditorResponse>>(
    `/slots/editor?date=${encodeURIComponent(date)}`,
  );
  return {
...
export async function getWeekSlots(date: string) {
  const data = await request<{ days?: WeekSlotDay[] | null }>(
    `/slots/week?date=${encodeURIComponent(date)}`,
  );
  const days = asArray(data.days).map((day) => ({
    ...day,
    slots: asArray(day?.slots),
  }));
  return { days };
}
...
export async function updateSlotColor(
  id: string,
  payload: { name: string; hex: string },
) {
...
export async function createSlotTemplate(
  payload: Pick<
    SlotTemplate,
    "weekday" | "startMinute" | "durationMinutes" | "colorPresetId" | "enabled"
  >,
) {
...
export async function createDaySlot(payload: {
  slotDate: string;
  startsAt: string;
  durationMinutes: number;
  colorPresetId: string;
  status: DailySlot["status"];
  note: string;
}) {
...
export async function moveDaySlot(payload: {
  id: string;
...
export async function deleteDaySlot(id: string) {
  return request<{ ok: boolean }>(`/slots/day-slots/${id}`, {
    method: "DELETE",
  });
}

export async function blockDaySlot(id: string) {
  return request<DailySlot>(`/slots/day-slots/${id}/block`, {
    method: "POST",
    body: JSON.stringify({}),
  });
}
...
export async function unblockDaySlot(id: string) {
  return request<DailySlot>(`/slots/day-slots/${id}/unblock`, {
    method: "POST",
    body: JSON.stringify({}),
  });
}
...
export async function getAvailableSlots(dateFrom: string, dateTo: string) {
  ...
  const data = await request<{ items?: DailySlot[] | null }>(
    `/slots/available?${search.toString()}`,
  );
  return { items: asArray(data.items) };
}
...
export async function getPublicCalendar(
  ...
  const data = await requestPublic<Partial<PublicCalendarResponse>>(
    `/public/calendar?${search.toString()}`,
  );
  return {
...
export async function updateAvailabilityExceptions(
  ...
  return request<{ exceptions: AvailabilityException[] }>(
...
export async function getBookings(status = "all") {
  ...
  const data = await request<{ items?: Booking[] | null }>(
    `/bookings${suffix}`,
  );
  return { items: asArray(data.items) };
}
...
export async function confirmBooking(
  ...
  return request(`/bookings/${id}/confirm`, {
    method: "POST",
    body: JSON.stringify({ amount, conversationId }),
  });
}
...
export async function getChannels() {
  const data = await request<{ items?: ChannelAccount[] | null }>(
    "/settings/channels",
  );
  return { items: asArray(data.items) };
}
...
export async function updateChannel(
  provider: string,
  payload: { connected: boolean; name: string },
) {
...
export async function getOperatorBotSettings() {
  const data = await request<OperatorBotSettings | null>(
    "/settings/operator-bot",
  );
  return (
    data ?? {
      binding: null,
