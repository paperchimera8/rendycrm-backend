...
func telegramButtonLabel(command string) string {
	if bookingID, action, position, ok := parseReminderToggleCommand(command); ok {
		_ = bookingID
		if action == "off" {
			return fmt.Sprintf("#%d Выкл", position)
		}
		return fmt.Sprintf("#%d Вкл", position)
	}
	switch {
...
func normalizeOperatorCommand(text string) string {
	if bookingID, action, _, ok := parseReminderToggleCommand(text); ok {
		return "reminder:toggle:" + bookingID + ":" + action
	}
	switch {
