...
func publicCalendarTokenExpiry(now time.Time) time.Time {
	if now.IsZero() {
		now = time.Now().UTC()
	}
	return now.UTC().Truncate(24 * time.Hour).Add(publicCalendarTokenTTL)
}
...
func (s *Server) encodePublicCalendarAccessToken(payload publicCalendarAccessToken) (string, error) {
	...
	if payload.ExpiresAt.IsZero() {
		payload.ExpiresAt = publicCalendarTokenExpiry(time.Now().UTC())
	}
	raw, err := json.Marshal(payload)
	if err != nil {
		return "", err
	}
	mac := hmac.New(sha256.New, []byte(s.cfg.EncryptionSecret))
	mac.Write(raw)
...
func (s *Server) clientCalendarURL(account ChannelAccount, chatID, workspaceID string) (string, error) {
	token, err := s.encodePublicCalendarAccessToken(publicCalendarAccessToken{
		...
		ExpiresAt:        publicCalendarTokenExpiry(time.Now().UTC()),
	})
	if err != nil {
		return "", err
	}
	baseURL := strings.TrimSpace(s.cfg.AppPublicBaseURL)
	if baseURL == "" {
...
func (s *Server) handlePublicCalendar(w http.ResponseWriter, r *http.Request) {
	...
	workspace, err := s.runtime.repository.WorkspaceByID(r.Context(), access.WorkspaceID)
	if err != nil {
